...
reached. Current Year: 1518. Current Location: North Pole Utility Closet 83N10." You made it! Now, to find
those anomalies.
...
working on a new kind of suit that would let him fit through tight spaces like that. But, I heard that a few
days ago, they lost the prototype fabric, the design plans, everything! Nobody on the team can even seem to
...
the box IDs should be similar. Too bad it would take forever to search the warehouse for two similar box
...
an ID containing exactly two of any letter and then separately counting those with exactly three of any
letter. You can multiply those two counts together to get a rudimentary checksum and compare it to what your
device predicts.
...
def calc_checksum(inpObj):
    """ Calculate the two-three checksum from the input """
    twocnt, threecnt = 0, 0
    for entry in inpObj:
       boxid = entry.strip()
       two, three = get_two_three(boxid)
       twocnt += two
       threecnt += three
...
def get_two_three(boxid):
    """ Return a tuple of (twoCnt, threeCnt) from one boxid """
    count_map = {}
    # populate count_map with counts of each letter
    for char in boxid:
        if char not in count_map:
            count_map[char] = 0
        count_map[char] += 1

    # Now count twos and threes
    twos, threes = 0, 0
